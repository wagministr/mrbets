...
class PreProcessor:
    ...
    async def store_to_supabase(self, match_id: int, source: str, content: str, 
                                vector: List[float], reliability: float) -> str:
        ...
        try:
            ...
            result = self.supabase.table("raw_events").insert(record).execute()
            ...
            if source == "api_football":
                ...
                try:
                    ...
                    if "fixtures" in data and data["fixtures"]:
                        ...
                        summary_parts.append(f"Match: {fixture.get('teams', {}).get('home', {}).get('name')} vs {fixture.get('teams', {}).get('away', {}).get('name')}")
